# Documentation Index

| Document | Use it for |
| --- | --- |
| [Feature Reference](./FEATURES.md) | Full tool, feature, shortcut, export, mobile, PWA, and accessibility reference preserved from the original README. |
| [Architecture](./ARCHITECTURE.md) | High-level React/Vite app structure, state flow, rendering, persistence, and test strategy. |
| [Development Guide](./DEVELOPMENT.md) | Local setup, npm scripts, validation commands, and test-suite organization. |
| [Roadmap](./ROADMAP.md) | Competitive analysis, completed work, and future feature direction. |
| [Sharing Guide](./SHARING.md) | Positioning, audiences, launch copy, demo scripts, repo metadata, and follow-up promotion backlog. |
| [Changelog](../CHANGELOG.md) | Notable project changes. |
| [Contributing](../CONTRIBUTING.md) | Setup, validation, development, and pull request expectations. |
| [Code of Conduct](../CODE_OF_CONDUCT.md) | Community behavior expectations. |
| [Pull Request Template](../PULL_REQUEST_TEMPLATE.md) | Checklist and context expected in pull requests. |
| [Issue Templates](../.github/ISSUE_TEMPLATE/) | Bug report and feature request forms. |
