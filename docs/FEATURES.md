...
- **Tokens with icon library** - drop Player, NPC, and Monster tokens (small 1×1, medium 2×2, large 3×3) onto the map with a searchable icon picker (legacy icons plus three opt-in Folio silhouettes); Move Token to drag, Remove Token to delete
...
- **Folio token reference** - Warden, Wayfinder and Drake silhouettes use a party shield, NPC round seal or hostile hexagon based on the token kind, not the chosen silhouette. Frames use shape/marks as well as color, with monochrome print companions. Existing token artwork stays unchanged. Open **The Lantern Watch** sample for the furnished reference; visual approval and the full twelve-silhouette kit remain pending.
