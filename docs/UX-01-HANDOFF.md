...
**Scope:** Bounded UX-01A implementation, not completion of UX-01 or authorization to start UX-02. UX-00 participant research remains explicitly deferred by the owner, with zero participants. UX-09 qualification gates are unchanged.
...
Validation allows legitimate off-grid notes, rooms, vectors, markers, and stair cells retained by existing resize behavior. **Known compatibility limit:** older projects with mismatched `fog` or `explored` grid dimensions are rejected, not cropped or silently repaired. The original remains downloadable. An explicit retained-original repair workflow is still needed for that historical malformed-grid case. New resizes keep explored dimensions aligned.
...
| `replacement-recovery` | Up to five previous committed projects retained when New, import, sample replacement, or explicit failed-restore recovery commits. Ordinary subsequent edits do not evict these copies. |
...
- `saveProject(project, expectedRevision, checkpoint?): Promise<string>`: validates, reads the current revision, retains recovery data, and writes in one readwrite transaction. Resolves only on transaction completion. An aborted transaction, including one whose put request succeeded first, is a failure.
...
New/import/sample replacement is paused while saving, after save failure, or during conflict. This prevents replacement from discarding newer in-memory work that has not committed. A saved predecessor is checkpointed when the replacement commits. Until then, the original remains the active persisted record.
...
Only successfully committed predecessors can be retained. Debounced work lost through a forced tab/process termination is not promised durable. An unsaved blank project is not labeled saved merely because the application loaded. This slice does not add lifecycle checkpoints for every destructive level operation, clear, or generation, nor a browsable history with retention management and restore previews.
...
```sh
npm run build
npm test -- src/utils/__tests__/projectSchema.test.ts src/utils/__tests__/exportProjectJSON.test.ts src/utils/__tests__/saveCoordinator.test.ts src/hooks/__tests__/savePersistence.test.tsx src/hooks/__tests__/mapStateUtils.test.ts src/hooks/__tests__/mapStatePerformance.test.ts src/hooks/__tests__/mapStateRivers.test.ts src/components/__tests__/saveTrust.test.tsx src/utils/__tests__/exportRender.test.ts
```

The production build succeeded; 120 cases across nine targeted files passed. Changed TypeScript/JavaScript files passed ESLint. Build retains the existing large-chunk warning. jsdom reports unsupported document navigation from download-link exercises; those cases pass and actual browser download events were also exercised.
...
- Complete non-destructive replacement boundaries across generation, clear, level deletion, and selected-region operations, including explicit checkpoint policy and scope messaging.
- Broader persisted history/recovery management and recovery previews, beyond one previous save and five replacement copies.
- Explicit recovery/repair for historical mismatched fog grids, retaining originals instead of silently dropping cells.
