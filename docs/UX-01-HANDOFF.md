...
- `listProjects()`: minimal catalog summaries with original values and per-record diagnostics. Both key/value reads use the `project:` key range, excluding checkpoint arrays and legacy archives from chooser enumeration. Invalid records remain downloadable, not hidden or silently repaired.
...
Failed startup also offers the chooser, without a placeholder name editor or canvas. A healthy alternate project can open without modifying the missing/corrupt selected record or its originals. The editor remains gated while loading that target. If it also fails, the original failure screen, source download, and chooser remain available.
...
A first save has no committed predecessor, even if it was requested by a checkpointed action. Coordinator capacity advances only when a committed predecessor actually existed; first-save clear/resize/restore cannot create a phantom count that pauses at 19 real copies.
