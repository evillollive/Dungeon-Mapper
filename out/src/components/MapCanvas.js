...
function drawToken(ctx, token, tileSize, isSelected = false) {
...
function drawAnnotation(ctx, stroke, tileSize) {
 ...
 * footprint. */
function findTokenAt(tokens, fx, fy) {
    if (!tokens)
        return null;
    ...
    for (let i = tokens.length - 1; i >= 0; i--) {
        ...
        if (tx >= t.x && tx < t.x + sz && ty >= t.y && ty < t.y + sz)
            return t;
    }
    return null;
}
const MapCanvas = forwardRef(({ map, activeTool, activeTile, themeId, printMode, viewMode, gmShowFog, selectedNoteId, selectedTokenId, drawColor, drawWidth, onSetTile, onSetTiles, onFillTile, onAddNote, onSelectNote, onEraseTiles, onSetFogCells, onAddToken, onMoveToken, onRemoveToken, onAddAnnotation, onRemoveAnnotation, onSelectionChange, }, ref) => {
    ...
    const [defogStroke, setDefogStroke] = useState(null);
    const lastDefogCellRef = useRef(null);
    ...
    useEffect(() => {
        ...
        for (const stroke of annotations) {
            if (isPlayerView && stroke.kind === 'gm')
                continue;
        ...
        if (renderFog) {
            ...
            for (let y = 0; y < meta.height; y++) {
                for (let x = 0; x < meta.width; x++) {
                    if (fog[y]?.[x]) {
                        if (defogSkip && defogSkip.has(`${x},${y}`))
                            continue;
        ...
        if (selection) {
            ...
            ctx.lineWidth = 1.5;
            ctx.setLineDash([4, 4]);
            ctx.strokeRect(selection.x * tileSize, selection.y * tileSize, selection.w * tileSize, selection.h * tileSize);
            ctx.setLineDash([]);
            ctx.restore();
        }
    ...
    const getFractionalCoords = useCallback((e) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return null;
        const rect = canvas.getBoundingClientRect();
        ...
        if (fx < 0 || fx > meta.width || fy < 0 || fy > meta.height)
            return null;
        return { x: fx, y: fy };
    }, [tileSize, meta.width, meta.height]);
    ...
    const handleCanvasAction = useCallback((e) => {
        ...
        if (activeTool === 'token-player' || activeTool === 'token-npc'
            ...
            const kind = activeTool === 'token-player' ? 'player'
            ...
            if (isPlayerView && fogActive && fog?.[y]?.[x])
                return;
            // Reject if the map is too small to fit the footprint at all.
            if (size > meta.width || size > meta.height)
                return;
    ...
    const handleFitToScreen = useCallback(() => {
        ...
        const controlsEl = wrapper?.querySelector('.zoom-controls');
                                ...
                                : activeTool === 'move-token' ? 'grab'
                                    : activeTool === 'remove-token' ? 'not-allowed'
                                        : activeTool === 'token-player' || activeTool === 'token-npc'
                                            ...
                                            : 'crosshair';
    return (_jsxs("div", { className: "canvas-wrapper", ref: containerRef, children: [_jsx("div", { className: "canvas-viewport", ref: viewportRef, children: _jsx("div", { className: "canvas-transform-container", style: {
                        transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`,
                        transformOrigin: 'center center',
                    }, children: _jsx("canvas", { ref: canvasRef, style: {
                            cursor: cursorStyle,
                            display: 'block',
                        }, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, onContextMenu: e => e.preventDefault(), onWheel: handleWheel }) }) }), _jsxs("div", { className: "zoom-controls", title: "Hold Shift + scroll to pan around the map", children: [_jsx("button", { onClick: () => setZoom(z => Math.min(4, z + 0.25)), children: "+" }), _jsxs("span", { children: [Math.round(zoom * 100), "%"] }), _jsx("button", { onClick: () => setZoom(z => Math.max(0.25, z - 0.25)), children: "-" }), _jsx("button", { onClick: () => { setZoom(1); setPan({ x: 0, y: 0 }); }, children: "Reset" }), _jsx("button", { onClick: handleFitToScreen, title: "Fit map to screen", children: "Fit" }), _jsx("span", { className: "zoom-hint", children: "\u21E7+wheel: pan" })] }), mousePos && (_jsxs("div", { className: "coord-display", children: ["X: ", mousePos.x, " \u00A0 Y: ", mousePos.y] })), _jsx("canvas", { ref: minimapRef, className: "minimap-canvas", onClick: handleMinimapClick, title: "Minimap - click to pan" })] }));
});
