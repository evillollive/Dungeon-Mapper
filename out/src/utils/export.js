...
export function importMapJSON(file) {
...
export function exportMapPNG(canvas, name, printFriendly = false) {
    if (printFriendly) {
 ...
 */
export function exportMapSVG(map, theme, resolveTheme, opts = {}) {
    const viewMode = opts.viewMode ?? 'gm';
    ...
    const isFogged = (x, y) => fogActive && !!fog?.[y]?.[x];
    ...
    for (let x = 0; x <= width; x++)
        svg += `<line x1="${x * tileSize}" y1="0" x2="${x * tileSize}" y2="${svgH}"/>`;
    for (let y = 0; y <= height; y++)
        svg += `<line x1="0" y1="${y * tileSize}" x2="${svgW}" y2="${y * tileSize}"/>`;
    svg += `</g>`;
    // Notes: hide notes under fog in player exports.
    map.notes.forEach(note => {
        if (isPlayerView && isFogged(note.x, note.y))
            return;
    ...
    for (const token of map.tokens ?? []) {
        if (isPlayerView && isFogged(token.x, token.y))
            continue;
