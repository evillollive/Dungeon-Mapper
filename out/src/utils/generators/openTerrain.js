import { collectCells, clampDensity, DIRS_4, getCell, makeTypeGrid, reorderNotesReadingOrder, setCell, typeGridToTiles, } from './common';
import { assignAreaKinds, detectAreas, getOpenAreaPalette } from './naturalAreas';
 ...
 */
function paintBlob(grid, rng, cx, cy, size, tile) {
    const h = grid.length;
 ...
 */
export function generateOpenTerrain(ctx) {
    ...
    const treasureCount = Math.min(floors.length, ov.treasure !== undefined
    ...
    const defaultTrapCount = Math.max(0, Math.round((mapArea / 400) * d * flavor.trapMultiplier));
    const trapCount = Math.min(floors.length, ov.trap !== undefined
    ...
    const isOpenForArea = (t) => t === 'floor' || t === 'water' || t === 'pillar' ||
    ...
    const areaKinds = keptAreas.length > 0
    ...
    for (let i = 0; i < pois.length; i++) {
        ...
        const insideLabeledArea = keptAreas.some(a => a.cells.some(c => c.x === p.x && c.y === p.y));
