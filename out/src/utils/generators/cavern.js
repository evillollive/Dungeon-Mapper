import { bfsDistances, clampDensity, collectCells, DIRS_8, getCell, makeTypeGrid, outlineWalls, reorderNotesReadingOrder, setCell, typeGridToTiles, } from './common';
import { assignAreaKinds, detectAreas, getCavernAreaPalette } from './naturalAreas';
import { getCavernFlavor, poiLabelFor, poiLabelIsRoom } from './poi';
import { makeRng } from './random';
/** Count the number of `wall` neighbors (8-connected) around `(x, y)`. */
function wallNeighbors(grid, x, y) {
    let n = 0;
 ...
 */
function carveWaterPool(grid, rng, cx, cy, size) {
    let x = cx;
    let y = cy;
 ...
 */
function paintWaterPools(grid, rng, count, avgSize) {
    if (count <= 0)
        return;
 ...
 */
function pickBiasedFloor(cells, areaIdByCell, areaKinds, bias, width, rng) {
    if (cells.length === 0)
 ...
 */
export function generateCavern(ctx) {
    ...
    if (bestRegion.length === 0) {
        ...
        const tilesEmpty = typeGridToTiles(grid);
    ...
    if (placeStairs) {
        const { farthest } = bfsDistances(grid, start.x, start.y, t => t === 'floor' || t === 'start' || t === 'water');
    ...
    const isOpenForArea = (t) => t === 'floor' || t === 'water' || t === 'start' || t === 'stairs-down' ||
    ...
    const areaCountTarget = ov.areas !== undefined
        ...
        : defaultAreaCount;
    const sortedAreas = [...detected].sort((a, b) => b.area - a.area);
    const keptAreas = areaCountTarget > 0
    ...
    for (let i = 0; i < pois.length; i++) {
        ...
        const kind = labelIsRoom && !insideLabeledArea ? 'room' : 'poi';
        notes.push({
            id,
            x: p.x,
            y: p.y,
    ...
    if (keptAreas.length > 0) {
        ...
        for (let i = 0; i < keptAreas.length; i++) {
            ...
            if (tiles[ay]?.[ax]?.noteId !== undefined) {
                ...
                for (const c of area.cells) {
                    if (tiles[c.y]?.[c.x]?.noteId !== undefined)
                        continue;
                    const dx = c.x - mx;
                    const dy = c.y - my;
