 ...
 */
export const DEFAULT_POI_LABELS = {
    start: 'Entrance',
...
export function getRoomsCorridorsFlavor(themeId) {
    if (!themeId)
        return DEFAULT_ROOMS_FLAVOR;
    const overrides = ROOMS_FLAVORS[themeId];
    if (!overrides)
        return DEFAULT_ROOMS_FLAVOR;
    return { ...DEFAULT_ROOMS_FLAVOR, ...overrides };
}
const DEFAULT_CAVERN_FLAVOR = {
    trapMultiplier: 1,
    treasureMultiplier: 1,
    waterMultiplier: 1,
    placeStairsDown: true,
};
const CAVERN_FLAVORS = {
...
export function getCavernFlavor(themeId) {
    if (!themeId)
        return DEFAULT_CAVERN_FLAVOR;
    const overrides = CAVERN_FLAVORS[themeId];
    if (!overrides)
        return DEFAULT_CAVERN_FLAVOR;
    return { ...DEFAULT_CAVERN_FLAVOR, ...overrides };
}
const OPEN_TERRAIN_FLAVORS = {
...
const DEFAULT_OPEN_FLAVOR = { treasureCount: 1, trapMultiplier: 1 };
export function getOpenTerrainFlavor(themeId) {
    if (!themeId)
        return DEFAULT_OPEN_FLAVOR;
 ...
 * those generators.
 */
const GENERATOR_THEME_POI_LABELS = {
    cavern: {
        dungeon: {
            start: 'Cave Mouth',
...
function resolveLabelEntry(themeId, tile, generatorId) {
 ...
 * defaults.
 */
export function poiLabelFor(themeId, tile, index, generatorId) {
 ...
 */
export function poiLabelIsRoom(themeId, tile, generatorId) {
    const entry = resolveLabelEntry(themeId, tile, generatorId);
