...
export function makeTypeGrid(width, height, fill = 'empty') {
    return Array.from({ length: height }, () => Array.from({ length: width }, () => fill));
}
/** Convert a `TypeGrid` into the `Tile[][]` shape stored on `DungeonMap`. */
export function typeGridToTiles(grid) {
 ...
 */
export function outlineWalls(grid) {
    const h = grid.length;
 ...
 */
export function bfsDistances(grid, sx, sy, passable = t => t === 'floor') {
    const h = grid.length;
    const w = grid[0]?.length ?? 0;
...
export function collectCells(grid, type) {
    const out = [];
 ...
 */
export function reorderNotesReadingOrder(tiles, notes) {
    if (notes.length === 0)
        return notes;
    ...
    for (const n of sorted) {
        ...
        if (m)
            baseCounts.set(m[1], (baseCounts.get(m[1]) ?? 0) + 1);
    }
    const baseSeen = new Map();
    const relabeled = sorted.map(n => {
        const m = suffixRe.exec(n.label);
        if (!m)
            return n;
        const base = m[1];
    ...
    const idMap = new Map();
    const renumbered = relabeled.map((n, i) => {
        const newId = i + 1;
