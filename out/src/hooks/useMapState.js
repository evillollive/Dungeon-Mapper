 ...
 */
function nextIdAfter(items) {
    if (!items || items.length === 0)
        return 1;
    return Math.max(...items.map(i => i.id)) + 1;
}
export function useMapState() {
    const [map, setMap] = useState(createDefaultMap);
    ...
    const debouncedSave = useCallback((mapToSave) => {
    ...
    function pushHistory(currentTiles, currentFog) {
        const snap = {
            tiles: currentTiles,
            fog: currentFog ?? createFogGrid(currentTiles[0]?.length ?? 0, currentTiles.length, false),
        };
    ...
    const setTile = useCallback((x, y, type) => {
        setMap(prev => {
    ...
    const fillTiles = useCallback((x, y, fillType) => {
        setMap(prev => {
    ...
    const setTiles = useCallback((updates) => {
        setMap(prev => {
            pushHistory(prev.tiles, prev.fog);
            const newTiles = prev.tiles.map(row => row.map(t => ({ ...t })));
    ...
    const setMapName = useCallback((name) => {
        setMap(prev => {
    ...
    const resizeMap = useCallback((width, height) => {
        setMap(prev => {
            const newTiles = Array.from({ length: height }, (_, y) => Array.from({ length: width }, (_, x) => prev.tiles[y]?.[x] ?? { type: 'empty' }));
     ...
     */
    const generateMap = useCallback((tiles, width, height, notes = [], name) => {
        setMap(prev => {
            pushHistory(prev.tiles, prev.fog);
            const updated = {
                ...prev,
     ...
     */
    const applyGeneratedRegion = useCallback((genTiles, ox, oy, genNotes = []) => {
        ...
        setMap(prev => {
            ...
            for (const n of genNotes)
                idMap.set(n.id, n.id + idOffset);
            for (let y = 0; y < regionH; y++) {
                const ty = oy + y;
                if (ty < 0 || ty >= prev.meta.height)
                    continue;
                for (let x = 0; x < regionW; x++) {
                    const tx = ox + x;
                    if (tx < 0 || tx >= prev.meta.width)
                        continue;
                    const src = genTiles[y][x];
                    const next = { type: src.type };
                    if (src.noteId !== undefined) {
                        const remapped = idMap.get(src.noteId);
                        if (remapped !== undefined)
                            next.noteId = remapped;
                    }
                    newTiles[ty][tx] = next;
                }
            }
            const remappedNotes = genNotes.map(n => ({
                ...n,
                id: n.id + idOffset,
                x: n.x + ox,
                y: n.y + oy,
    ...
    const loadMapData = useCallback((loaded) => {
        const ready = withDefaults(loaded);
        pastRef.current = [];
        futureRef.current = [];
        setCanUndo(false);
    ...
    const updateNote = useCallback((id, label, description) => {
        setMap(prev => {
    ...
    const deleteNote = useCallback((id) => {
        setMap(prev => {
            ...
            const newTiles = prev.tiles.map(row => row.map(t => t.noteId === id ? { ...t, noteId: undefined } : t));
    ...
    const setTileSize = useCallback((tileSize) => {
        setMap(prev => {
    ...
    const setTheme = useCallback((theme, preserveExisting = false) => {
        setMap(prev => {
            ...
            if (preserveExisting) {
                ...
                let mutated = false;
                const stamped = prev.tiles.map(row => row.map(t => {
                    if (t.type === 'empty' || t.theme)
                        return t;
                    mutated = true;
                ...
                const { notes: themed, removedIds } = reThemeNotes(prev.notes, prev.tiles, theme);
                newNotes = themed;
                if (removedIds.size > 0) {
                    newTiles = prev.tiles.map(row => row.map(t => t.noteId !== undefined && removedIds.has(t.noteId)
                        ? { ...t, noteId: undefined }
                        : t));
                }
            }
    ...
    const setFogCells = useCallback((cells, hidden) => {
        setMap(prev => {
    ...
    const fillAllFog = useCallback((hidden) => {
        setMap(prev => {
            const w = prev.meta.width;
            const h = prev.meta.height;
            const current = prev.fog ?? createFogGrid(w, h, false);
    ...
    const setFogEnabled = useCallback((enabled) => {
        setMap(prev => {
            if ((prev.fogEnabled ?? false) === enabled)
                return prev;
     ...
     */
    const addToken = useCallback((kind, x, y, label, size) => {
    ...
    const moveToken = useCallback((id, x, y) => {
        setMap(prev => {
            ...
            if (cx === existing.x && cy === existing.y)
                return prev;
            const tokens = (prev.tokens ?? []).map(t => t.id === id ? { ...t, x: cx, y: cy } : t);
    ...
    const updateToken = useCallback((id, patch) => {
        setMap(prev => {
            const tokens = (prev.tokens ?? []).map(t => t.id === id ? { ...t, ...patch } : t);
            const updated = { ...prev, tokens };
            debouncedSave(updated);
            return updated;
        });
    ...
    const reorderInitiative = useCallback((fromIndex, toIndex) => {
        setMap(prev => {
            ...
            if (fromIndex < 0 || fromIndex >= init.length)
                return prev;
    ...
    const addAnnotation = useCallback((stroke) => {
    ...
    const clearAnnotations = useCallback((kind) => {
        setMap(prev => {
