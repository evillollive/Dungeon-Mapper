...
const NOOP_UPDATE_NOTE = () => { };
const NOOP_DELETE_NOTE = () => { };
const NOOP_ACTIVATE_NOTE_TOOL = () => { };
function loadInitialPreserveOnThemeSwitch() {
...
function App() {
    const { map, selectedNoteId, setSelectedNoteId, setTile, fillTiles, setTiles, setMapName, resizeMap, clearMap, newMap, loadMapData, generateMap, applyGeneratedRegion, addNote, updateNote, deleteNote, setTileSize, setTheme, undo, redo, canUndo, canRedo, setFogCells, fillAllFog, setFogEnabled, addToken, moveToken, removeToken, updateToken, reorderInitiative, clearInitiative, addAnnotation, removeAnnotation, clearAnnotations, } = useMapState();
    const { activeTool, setActiveTool, activeTile, setActiveTile, } = useDrawingTool();
    const canvasRef = useRef(null);
    const themeId = map.meta.theme ?? 'dungeon';
    const [printMode, setPrintMode] = useState(false);
    const [viewMode, setViewMode] = useState(loadInitialViewMode);
    const [uiScale, setUIScale] = useState(loadInitialUIScale);
    const [preserveOnThemeSwitch, setPreserveOnThemeSwitch] = useState(loadInitialPreserveOnThemeSwitch);
    const [gmShowFog, setGmShowFog] = useState(loadInitialGmShowFog);
    ...
    const [drawWidth, setDrawWidth] = useState(0.25);
    ...
    const handleAddToken = useCallback((kind, x, y, _label, size) => {
        if (viewMode === 'player') {
    ...
    const handleRenameToken = useCallback((id, label) => {
    ...
    const hasExistingContent = useMemo(() => map.tiles.some(row => row.some(t => t.type !== 'empty')) ||
        (map.notes?.length ?? 0) > 0 ||
        (map.tokens?.length ?? 0) > 0, [map.tiles, map.notes, map.tokens]);
    ...
    const handleGenerateMap = useCallback((result, suggestedName, target) => {
        if (target) {
    ...
    useEffect(() => {
        try {
            window.localStorage.setItem(PRESERVE_THEME_STORAGE_KEY, preserveOnThemeSwitch ? '1' : '0');
        }
        catch {
    ...
    const handleEraseTiles = useCallback((tiles) => {
        setTiles(tiles.map(t => ({ ...t, type: 'empty' })));
    }, [setTiles]);
    ...
    return (_jsxs("div", { className: "app", children: [_jsx(MapHeader, { map: map, onSetName: setMapName, onResize: resizeMap, onSetTileSize: setTileSize, onClear: clearMap, onNew: newMap, onLoad: loadMapData, onExportSVG: handleExportSVG, onUndo: undo, onRedo: redo, canUndo: canUndo, canRedo: canRedo, printMode: printMode, onTogglePrintMode: () => setPrintMode(p => !p), uiScale: uiScale, uiScaleOptions: UI_SCALE_OPTIONS, onSetUIScale: setUIScale, getCanvas: () => canvasRef.current?.getCanvas() ?? null, viewMode: viewMode, onToggleViewMode: switchViewMode }), _jsxs("div", { className: "app-body", children: [viewMode === 'gm' ? (_jsx(Toolbar, { activeTool: activeTool, activeTile: activeTile, themeId: themeId, onSetTool: setActiveTool, onSetTile: setActiveTile, onSetTheme: setTheme, preserveOnThemeSwitch: preserveOnThemeSwitch, onTogglePreserveOnThemeSwitch: () => setPreserveOnThemeSwitch(p => !p), fogEnabled: fogEnabled, gmShowFog: gmShowFog, onToggleGmShowFog: handleToggleGmShowFog, onOpenGenerateMap: handleOpenGenerateMap })) : (_jsx(PlayerToolbar, { activeTool: activeTool, onSetTool: setActiveTool, drawColor: drawColor, onSetDrawColor: setDrawColor, drawWidth: drawWidth, onSetDrawWidth: setDrawWidth, onClearPlayerDrawings: handleClearPlayerDrawings, fogEnabled: fogEnabled, onToggleFogEnabled: handleToggleFogEnabled, onResetFog: handleResetFog, onClearFog: handleClearFog })), _jsx("main", { className: "canvas-area", children: _jsx(MapCanvas, { ref: canvasRef, map: map, activeTool: activeTool, activeTile: activeTile, themeId: themeId, printMode: printMode, viewMode: viewMode, gmShowFog: gmShowFog, selectedNoteId: selectedNoteId, selectedTokenId: selectedTokenId, drawColor: drawColor, drawWidth: drawWidth, onSetTile: setTile, onSetTiles: setTiles, onFillTile: fillTiles, onAddNote: addNote, onSelectNote: setSelectedNoteId, onEraseTiles: handleEraseTiles, onSetFogCells: setFogCells, onAddToken: handleAddToken, onMoveToken: moveToken, onRemoveToken: removeToken, onAddAnnotation: addAnnotation, onRemoveAnnotation: removeAnnotation, onSelectionChange: setSelection }) }), viewMode === 'gm' && (_jsxs("aside", { className: "right-panel", children: [_jsx(InitiativePanel, { tokens: map.tokens ?? [], initiative: map.initiative ?? [], selectedTokenId: selectedTokenId, onSelectToken: setSelectedTokenId, onRenameToken: handleRenameToken, onReorder: reorderInitiative, onClear: clearInitiative, viewMode: "gm" }), _jsx(NotesPanel, { notes: map.notes, selectedNoteId: selectedNoteId, onSelectNote: handleSelectNote, onUpdateNote: updateNote, onDeleteNote: deleteNote, onActivateNoteTool: () => setActiveTool('note') })] })), viewMode === 'player' && (_jsxs("aside", { className: "right-panel", children: [_jsx(InitiativePanel
                            ...
                            , { 
                                ...
                                tokens: fogEnabled
                                    ...
                                    : (map.tokens ?? []), initiative: map.initiative ?? [], selectedTokenId: selectedTokenId, onSelectToken: setSelectedTokenId, 
                                ...
                                onRenameToken: handleRenameToken, onReorder: reorderInitiative, onClear: clearInitiative, viewMode: "player" }), _jsx(NotesPanel
                            ...
                            , { 
                                ...
                                notes: fogEnabled
                                    ...
                                    : map.notes, selectedNoteId: selectedNoteId, onSelectNote: handleSelectNote, onUpdateNote: NOOP_UPDATE_NOTE, onDeleteNote: NOOP_DELETE_NOTE, onActivateNoteTool: NOOP_ACTIVATE_NOTE_TOOL })] }))] }), showGenerateDialog && viewMode === 'gm' && (_jsx(GenerateMapDialog, { themeId: themeId, initialWidth: map.meta.width, initialHeight: map.meta.height, hasExistingContent: hasExistingContent, selection: selection, onCancel: handleCancelGenerateMap, onGenerate: handleGenerateMap }))] }));
}
export default App;
