...
export default async function runArtRenderReview(page) {
  ...
  try {
    ...
    const measurements = await tab.evaluate(async () => {
      const { buildFolioReference } = await import('/Dungeon-Mapper/src/utils/folioReference.ts');
      ...
      render('After / Dungeon Folio v1', map);
      ...
      const title = document.createElement('h1'); title.textContent = 'The Quiet Cistern / Dungeon Folio v1';
      const subtitle = document.createElement('p'); subtitle.textContent = 'Same geometry. Quiet floors, continuous walls, explicit player publication. Art approval pending.';
      ...
      return { coldRenderMs: cold, warmMedianMs: timings[5], warmMaxMs: timings.at(-1), cacheEntries: folioCacheSize(), dimensions: '128 x 128', tileSize: 16 };
    });
    await tab.screenshot({ path: 'docs/media/ux07/contact-sheet.png', fullPage: true });
    await tab.evaluate(() => {
      document.querySelector('.sheets').style.display = 'none';
      document.querySelector('#zooms').style.display = 'block';
    });
    await tab.screenshot({ path: 'docs/media/ux07/zoom-sheet.png', fullPage: true });
