...
import type { TypeGrid } from './common';

/** A single archetype entry in a natural-area palette. */
export interface NaturalAreaKind {
  label: string;
  weight?: number;
  /** Bias the largest detected area to a `'large'` archetype when one exists. */
  size?: 'large' | 'medium' | 'small';
  /** Optional treasure / trap multipliers consumed by the cavern generator. */
  bias?: { treasure?: number; trap?: number };
  /**
   * Optional terrain hint — when set, the generator tries to assign this
   * archetype to chambers that contain at least one cell of the matching
   * tile (e.g. "Underground Pool" prefers chambers with `water`).
   */
  prefersTerrain?: TileType;
}

export type NaturalAreaPalette = NaturalAreaKind[];

/* ── Cavern palettes (per theme) ─────────────────────────────── */

const CAVERN_DEFAULT: NaturalAreaPalette = [
...
const CAVERN_DUNGEON: NaturalAreaPalette = [
  { label: 'Grand Chamber', size: 'large', weight: 1, bias: { treasure: 1.5 } },
  { label: 'Side Gallery', weight: 1.2 },
  { label: 'Antechamber', weight: 1 },
  { label: 'Stalagmite Forest', weight: 0.8, bias: { trap: 1.2 } },
...
const CAVERN_WILDERNESS: NaturalAreaPalette = [
...
const CAVERN_ANCIENT: NaturalAreaPalette = [
...
const CAVERN_STARSHIP: NaturalAreaPalette = [
...
const CAVERN_ALIEN: NaturalAreaPalette = [
...
const CAVERN_PIRATE: NaturalAreaPalette = [
...
const CAVERN_DESERT: NaturalAreaPalette = [
...
const CAVERN_POSTAPOCALYPSE: NaturalAreaPalette = [
...
const CAVERN_CYBERPUNK: NaturalAreaPalette = [
...
const CAVERN_OLDWEST: NaturalAreaPalette = [
...
const CAVERN_STEAMPUNK: NaturalAreaPalette = [
...
const CAVERN_CASTLE: NaturalAreaPalette = [
...
const CAVERN_MODERNCITY: NaturalAreaPalette = [
...
export const CAVERN_AREA_PALETTES: Record<string, NaturalAreaPalette> = {
...
export function getCavernAreaPalette(themeId?: string): NaturalAreaPalette {
  if (!themeId) return CAVERN_DEFAULT;
  return CAVERN_AREA_PALETTES[themeId] ?? CAVERN_DEFAULT;
}

/* ── Open-terrain palettes (per theme) ───────────────────────── */

const OPEN_DEFAULT: NaturalAreaPalette = [
...
const OPEN_WILDERNESS: NaturalAreaPalette = [
  { label: 'Great Clearing', size: 'large', weight: 1 },
...
const OPEN_DESERT: NaturalAreaPalette = [
...
const OPEN_POSTAPOCALYPSE: NaturalAreaPalette = [
...
const OPEN_OLDWEST: NaturalAreaPalette = [
...
const OPEN_PIRATE: NaturalAreaPalette = [
...
const OPEN_ALIEN: NaturalAreaPalette = [
...
const OPEN_ANCIENT: NaturalAreaPalette = [
...
export const OPEN_AREA_PALETTES: Record<string, NaturalAreaPalette> = {
...
export function getOpenAreaPalette(themeId?: string): NaturalAreaPalette {
  if (!themeId) return OPEN_DEFAULT;
...
/**
 * A detected natural area. `cells` is the set of "interior" cells
 * (those that satisfied the radius threshold); `area` is `cells.length`
 * for convenience. `centroid` is guaranteed to be one of `cells` so
 * callers can anchor a `MapNote` directly on it without worrying about
 * landing on a wall.
 */
...
export function detectAreas(
  ...
  while (head < queue.length) {
    ...
    for (const [dx, dy] of dirs8) {
      ...
      if (dist[ny][nx] !== -1 && dist[ny][nx] <= d + 1) continue;
      dist[ny][nx] = d + 1;
      queue.push([nx, ny]);
    }
  }
  ...
  const interior: boolean[][] = Array.from({ length: h }, () => new Array(w).fill(false));
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (isOpen(grid[y][x]) && dist[y][x] >= minRadius) interior[y][x] = true;
    }
  }
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (!interior[y][x] || visited[y][x]) continue;
      ...
      while (stack.length > 0) {
        ...
        for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]] as const) {
          ...
          const ny = cy + dy;
          if (nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
          if (visited[ny][nx] || !interior[ny][nx]) continue;
          visited[ny][nx] = true;
          stack.push([nx, ny]);
        }
      }
...
function pickKind(palette: readonly NaturalAreaKind[], rng: Rng, predicate?: (k: NaturalAreaKind) => boolean): NaturalAreaKind {
 ...
 */
export function assignAreaKinds(
  areas: DetectedArea[],
  palette: readonly NaturalAreaKind[],
  grid: TypeGrid,
  rng: Rng
): (NaturalAreaKind | undefined)[] {
  const result: (NaturalAreaKind | undefined)[] = areas.map(() => undefined);
  if (areas.length === 0 || palette.length === 0) return result;
