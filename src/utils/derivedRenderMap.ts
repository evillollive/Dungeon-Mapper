import type { DungeonMap, Tile } from '../types/map';
...
export function deriveRenderableTiles(map: DungeonMap): Tile[][] {
  const roomShapes = map.roomShapes ?? [];
  if (roomShapes.length === 0) return map.tiles;
  return rasterizeRoomShapes(map.tiles, roomShapes, map.meta.width, map.meta.height);
}
