...
export function buildKeyBindings(actions: ShortcutActions): KeyBinding[] {
  ...
  return [
    ...
    tool('tool.roomRect', 'q', 'Room Rectangle tool', 'room-rect'),
    {
      id: 'tool.roomCircle',
      category: 'Tools',
      keys: 'Shift+C',
      description: 'Room Circle tool — drag to draw circular/elliptical room shapes',
      match: e => isPlainKey(e) && e.shiftKey && e.key.toLowerCase() === 'c',
      action: () => actions.setActiveTool('room-circle'),
    },
    {
      id: 'tool.roomPoly',
      category: 'Tools',
      keys: 'Shift+P',
      description: 'Room Polygon tool — click to add vertices, double-click to close',
      match: e => isPlainKey(e) && e.shiftKey && e.key.toLowerCase() === 'p',
      action: () => actions.setActiveTool('room-poly'),
    },
    {
      id: 'tool.roomCut',
      category: 'Tools',
      keys: 'Shift+Q',
      description: 'Room Cut tool — draw subtractive shapes that carve out room geometry',
      match: e => isPlainKey(e) && e.shiftKey && e.key.toLowerCase() === 'q',
      action: () => actions.setActiveTool('room-cut'),
    },
